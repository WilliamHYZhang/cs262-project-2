...
Run client with: `python3 vm.py --id=1 --trial=1` or `python3 supervisor.py`

Generate graphs with: `python3 gen_graphs.py`
...
- Some trials displayed closely aligned logical clocks (e.g. Trial 3) due to similar tick rates and message exchanges, whereas others exhibited one VM outpacing the rest (e.g. Trial 4).
...
- In **Figure 3**, message queues remained mostly near zero but occasionally spiked significantly (up to ~15 messages) during rapid bursts of incoming messages.
...
- **Closer Ranges Across VMs:**  
  Because the clock rates are all between 1 and 2, the average inter‐event times for the three VMs typically fall within a narrower band—often around 0.6 s to 1.0 s.
...
- **Message Queue Dynamics:** Fluctuations indicated asynchronous processing, affecting logical clock adjustments due to message backlog handling. Tick rate differences significantly influence logical clock progression, message traffic patterns, and queue congestion.